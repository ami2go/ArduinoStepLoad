...
#include "TimerTwo.h"     //https://github.com/theAndreas/TimerTwo
#include "ClickEncoder.h" //https://www.mikrocontroller.net/articles/Drehgeber
...
#include "SSD1306Ascii.h"        //**** Version for Arduino Custome I2C Lib
...
#include "UserApplications.h" // sparated file for large code application 
#include "Variables_def.h"    // Constant and varialble collection for the project  
...
int volatile  Nstarts=0;

// put string constant into FLASH memory to reduse RAM usage

#define strlenth 55        
const static char StrNext_flash[strlenth] PROGMEM = "Next: <Clikc>     Exit Menu: <Hold> "; 
...
const static char StrEdit_flash[strlenth] PROGMEM = "Edit      Settings:     <Hold>   ----->";
...
void TimerOneISR();
void WriteYellowLine (char Str1[30],char Str2[30] );
...
void enter_waiting_start(){
    ...
    char Str1_temp[strlenth];
    char Str2_temp[strlenth];
    byte k;
    int len;
    len = strlen_P(StrStart_flash);
    for(k=0; k<len; k++ ){
      Str1_temp[k] = pgm_read_byte_near(StrStart_flash + k);
         };

    len = strlen_P(StrEdit_flash);
    for(k=0; k<len; k++ ){
      Str2_temp[k] = pgm_read_byte_near(StrEdit_flash + k);
    };

     ScreenUpdate(Str1_temp, Str2_temp); 
    
     oled1.set1X();                  // updated
     oled1.setCursor(94,0);         // run counter
     oled1.clearToEOL();
      oled1.setCursor(105,0);         // run counter 
     oled1.print(Nstarts);           // on Oled screen 
     
     CHAvar.counter = CHAvar.ONtime + CHAvar.OFFtime ;
     CHAvar.Ncycles_counter = CHAvar.Ncycles ;
     Vout.Enable = 1 ; // ring buffer for Vout enable 
};
...
void enter_setup_ontime(){
    ...
    byte k;
    int len;
    len = strlen_P(StrNext_flash);
    for(k=0; k<len; k++ ){
      Str1_temp[k] = pgm_read_byte_near(StrOn_flash + k);
      Str2_temp[k] = pgm_read_byte_near(StrNext_flash + k);
    }; 

    WriteYellowLine(Str1_temp,Str2_temp);

    
  //  WriteYellowLineP(2,(__FlashStringHelper *)pgm_read_word(&Str2_ontime));
...
void enter_setup_offtime(){
  ...
  //store string in flash to free up some RAM
    ...
    len = strlen_P(StrNext_flash);
    for(k=0; k<len; k++ ){
      Str1_temp[k] = pgm_read_byte_near(StrOff_flash + k);
      Str2_temp[k] = pgm_read_byte_near(StrNext_flash + k);
    }; 
...
void enter_setup_ncycles(){
 //   DEBUG_PRINTLN("enter_setup_ncycles");
    ...
    len = strlen_P(StrNext_flash);
    for(k=0; k<len; k++ ){
      Str1_temp[k] = pgm_read_byte_near(StrNrep_flash + k);
      Str2_temp[k] = pgm_read_byte_near(StrNext_flash + k);
    }; 
...
void WriteYellowLine (char Str1[strlenth],char Str2[strlenth] ){
